...
## توضیح کامل + مثال هر مفهوم (همونی که در پلن اولیه «بعداً اضافه می‌شه» بود)

- ✅ همه‌ی مفاهیم **Market** (Market Cycle, Trend, Momentum, Thrust, Pullback, Range, Reversal, Continuation, Liquidity, Volume, Entry Edge) حالا توضیح کامل + مثال دارن؛ با باز کردن هر نود می‌بینیشون
- ✅ همه‌ی مفاهیم **Knowledge** (POC, VAH, VAL, HVN, LVN, Examples, Checklist) همین‌طور
- ✅ هر آیتم چک‌لیست **Execution** یک آیکن اطلاعات (ⓘ) کنارش داره که با کلیک، توضیح همون مرحله رو باز می‌کنه

## صف همگام‌سازی آفلاین

- ✅ اگه ثبت یک آیتم (معامله، تحلیل، چک‌این، یادداشت) وقتی آفلاینی یا Supabase در دسترس نیست شکست بخوره، به‌جای گم‌شدن، توی IndexedDB مرورگر صف می‌شه (`src/lib/outbox.ts`)
- ✅ با برگشت اینترنت یا با هر بار باز کردن اپ، صف خودکار تلاش می‌کنه دوباره بفرسته (`flushOutbox`)
- ✅ توی Topbar، اگه چیزی توی صفه یه نشان «X در صف» می‌بینی که با کلیک روش می‌تونی الان دستی سینک رو امتحان کنی
- این یعنی الان می‌تونی حتی وسط قطعی اینترنت خیالت راحت باشه که چک‌این یا ژورنالت گم نمی‌شه — با رفرش بعدی خودش می‌ره

## اتصال خودکار Risk به معاملات واقعی

- ✅ فیلد «ریسک این معامله (%)» به فرم Journal اضافه شد و توی دیتابیس ذخیره می‌شه (`risk_pct` روی `journal_entries`)
- ✅ «ریسک استفاده‌شده‌ی امروز» توی صفحه‌ی Risk (و در نتیجه روی Dashboard) دیگه دستی نیست — خودکار از مجموع ریسک معاملات همون روز توی Journal محاسبه می‌شه و هر چند دقیقه به‌روز می‌شه
- ✅ توی محاسبه‌گر حجم معامله، دکمه‌ی «استفاده از X% در ثبت معامله‌ی بعدی» هست — بزنیش، همون درصد به‌عنوان پیش‌فرض توی فرم Journal پر می‌شه
- ⚠️ اگه قبلاً `journal_entries` رو ساخته بودی، دوباره `supabase/schema.sql` رو کامل اجرا کن تا ستون `risk_pct` اضافه بشه
